...
<head>
  ...
  <?php include 'template/headinc.php'; ?>
		...
		$qs=simplexml_load_file("http://lda.data.parliament.uk/commonsoralquestions.xml?_view=Commons+Oral+Questions&AnswerDate=2017-07-18&_pageSize=500");
...
<!-- Here's the script that *should* get the relevant members from the search. Note search string must be greater than 2 -->		
		<script>
			function showResult(str) {
			  ...
			  xmlhttp.open("GET","template/qsearch.php?&date=<?php echo $date ?>",true);
			  xmlhttp.send();
			}
			</script>
    ...
    <div class="row">
      ...
      <div class="col-sm-4 bootcards-list" id="list" data-title="Contacts">
		...
		<?php 
			...
			if (!$q){
				echo file_get_contents("template/initialqs.php"); 
            }
            else {
				$qselected = '&mselected='.$q;
				echo file_get_contents("http://leedhammedia.com/parliament/test/template/qsearch.php?q=".$q."&date=".$date); 
			} 
