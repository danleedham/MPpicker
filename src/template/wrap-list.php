...
$xmlDoc->load('http://lda.data.parliament.uk/tvprogrammes/'.$location.'.xml');

// First we need to find the times which the program covers (likely it doesn't have an end date)
$y=$xmlDoc->getElementsByTagName('itemListElement');
$y=$y->item(0)->getElementsByTagName('item');

// Find the item list element with postition that equals section

// 

$loggedcount = $y->length;

$x=$xmlDoc->getElementsByTagName('tvClip');
