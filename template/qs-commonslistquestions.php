...
$xmlDoc=new DOMDocument();
	...
	$xmlDoc->load('http://lda.data.parliament.uk/commonsoralquestions.xml?_view=Commons+Oral+Questions&AnswerDate='.$date.$qtypeURL.$qdeptURL.'&_pageSize=500&_properties=unstarredQuestion');
	...
	if ($length !== 0) {
		...
		for($i=0; $i < $length; $i++) {
			if ($qarray[$i]['QuestionStatus'] == "Withdrawn Without Notice" or $qarray[$i]['unstarredQuestion'] == "true" ){	
			} else {
